...
def pick_earliest_version(versions: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    for v in versions:
        ...
        if not y:
            continue
        yi = int(y)
        # Prefer smaller year; tie-breaker: vinyl formats or 'Main Release' if indicated
        if yi < best_year:
            best = v; best_year = yi
...
def get_original_release_info(master_id: int, min_img_size: int) -> Tuple[str, str, int]:
    ...
    img_url = ""
    if release_id:
        # fetch full release to get robust labels & images
        try:
...
def find_audio_files(root: Path, recursive: bool) -> List[Path]:
    # Dedup + case-insensitive suffix filtering (avoids double counting)
...
def process_one_file(f: Path, args, placeholder_md5: Optional[str]) -> Dict[str, Any]:
    ...
    # ORIGINAL release label + (if needed) artwork from earliest version
    ...
    # Also keep matched release images as an emergency fallback (if we didn't get orig image)
    ...
    if not args.no_art and f.suffix.lower() in art_supported_exts and PIL_AVAILABLE:
        ...
        # If we need art, prefer original release art; else fall back to matched release art
        if need_art:
            ...
            if candidate_img:
                ...
                if img_bytes:
                    ...
                    if write_single_cover(f, img_bytes, mime=mime):
                        try: w2,h2 = image_size_from_bytes(img_bytes); art_status=f"downloaded ({w2}x{h2}) from original release due to {reason}" if candidate_img==orig_img_url else f"downloaded ({w2}x{h2}) due to {reason}"
                        except Exception: art_status=f"downloaded from original release due to {reason}" if candidate_img==orig_img_url else f"downloaded due to {reason}"
